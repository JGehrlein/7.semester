...
plot(df_13_bac$Time[2:9],df_13_bac$Surv[2:9], pch = 16, cex = 1.2, xlab = 'Heatstress (hours)', 
     ...
     main = expression('Comparision between fly gut 13 without and with living bacteria'),
...
plot(df_13_bac$Time[2:9],df_13_bac$Surv[2:9], pch = 16, cex = 1.2, xlab = 'Heatstress (hours)', 
     ylab = 'Surviving fraction', col = 'firebrick',
     main = expression('Comparision between fly gut 13 without and with living bacteria'),
     xlim = c(0,22), ylim = c(0,1), xaxp = c(0,22,11))
lines(df_13_bac$Time,df_13_bac$Surv, col = 'firebrick',)
arrows(df_13_bac$Time, df_13_bac$Surv-df_13_bac$Std.error, df_13_bac$Time, 
       df_13_bac$Surv+df_13_bac$Std.error, length=0.05, angle=90, code=3, 
       col = 'firebrick')
points(df_13_meta$Time[2:9],df_13_meta$Surv[2:9], pch = 1, cex = 1.2, 
       col = 'firebrick' ) 
lines(df_13_meta$Time,df_13_meta$Surv, lty = 2, col = 'firebrick')
...
points(df_20_bac$Time[2:9],df_20_bac$Surv[2:9], pch = 16, cex = 1.2,
       col = 'blue') 
lines(df_20_bac$Time,df_20_bac$Surv,col = 'blue')
arrows(df_20_bac$Time[2:9], df_20_bac$Surv[2:9]-df_20_bac$Std.error[2:9], df_13_bac$Time[2:9], 
       df_20_bac$Surv[2:9]+df_20_bac$Std.error[2:9], length=0.05, angle=90, code=3, col = 'blue')
points(df_20_meta$Time[2:9],df_20_meta$Surv[2:9], pch = 1, cex = 1.2,
       col = 'blue') 
lines(df_20_meta$Time,df_20_meta$Surv, lty = 2,col = 'blue')
...
legend('topright', pch = c(16,1,16,1,16), lty = c(1,2,1,2,1), bty = 'n',
       col = c('firebrick','firebrick','blue','blue', 'black'),
legend = c(expression('Bacteria fly gut' ~ 13*degree*C),
           expression('Metabolite fly gut' ~ 13*degree*C),
           expression('Bacteria fly gut' ~ 20*degree*C),
           expression('Metabolite fly gut' ~ 20*degree*C),
           'OP50'))
```
