...
This is done by extracting the surviving proportion, standard errors from survfit().  and adding a startpoint where the survival is 100% at 0 hours.

```{r}
df_fly <- data.frame(c(0,km$time[1:5]),c(1,km$surv[1:5]),c(0,km$std.err[1:5]))
colnames(df_fly) <- c('Time','Surv','Std.error')
df_OP50 <- data.frame(c(0,km$time[6:10]),c(1,km$surv[6:10]),c(0,km$std.err[6:10]))
...
plot(df_fly$Time[2:6],df_fly$Surv[2:6], pch = 16, cex = 1.2, xlab = 'Heatstress (hours)', 
     ...
     xlim = c(0,16), ylim = c(0,1), xaxp = c(0,16,16))
